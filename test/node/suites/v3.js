import { ModelTestSuite, PipelineTestSuite } from "@benchmarking/core-v3";
...
for (const TestSuite of Suites) {
	...
	for await (const { name, result } of suite.run()) {
		...
		for (const [benchmark, stats] of Object.entries(result.stats)) {
	...
	await mkdir(Results).catch(() => console.info(`Folder "${Results}" already exists.`));
	await writeFile(`${Results}/${TestSuite.name}-${Version}.csv`, frame.join("\n"));
